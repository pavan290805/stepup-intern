...
async function getHandler(request: NextRequest, context?: RouteContext) {
  ...
  return communityController.getPost(request, { params: context?.params });
}

async function putHandler(request: NextRequest, context: AuthContext) {
  await connectToDatabase();
  return communityController.updatePost(request, context);
}

async function deleteHandler(request: NextRequest, context: AuthContext) {
  await connectToDatabase();
  return communityController.deletePost(request, context);
}

export const GET = withSecurityHeaders(withRequestLogger(withErrorHandler(getHandler)));
export const PUT = withSecurityHeaders(withRequestLogger(withErrorHandler(withAuth(putHandler))));

...
async function postHandler(request: NextRequest, context: AuthContext) {
  ...
  return mentorController.completeSession(request, context);
}

export const POST = withSecurityHeaders(withRequestLogger(withErrorHandler(withAuth((r, c) => withRole([ROLES.MENTOR], postHandler)(r, c)))));

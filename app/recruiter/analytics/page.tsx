...
import Navbar from "@/components/navigation/RecruiterNavbar";
import { apiFetch, type InternshipApiItem, type RecruiterProfileApi } from "@/lib/api";
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    const loadAnalytics = async () => {
      try {
        const profileResponse = await apiFetch<RecruiterProfileApi | null>("/recruiters/profile");
        const company = profileResponse?.data?.companyId;
        const companyId =
          typeof company === "object" && company
            ? company._id || (company as { id?: string }).id
            : company;
        ...
        const internshipsResponse = await apiFetch<{ internships: InternshipApiItem[] }>(`/internships?company=${companyId}&limit=100`);
